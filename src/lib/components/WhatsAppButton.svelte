<script lang="ts">
	...
	let { phone, name, amount, disabled = false, details, proofUrl, transferInfo, notifyStatus = 'pending', onNotified, compact = false }: {
		phone: string;
		name: string;
		amount: number;
		disabled?: boolean;
...
<button
	...
	class="inline-flex items-center rounded-lg text-xs font-medium transition-all
		{compact ? 'gap-0 px-2 py-1.5' : 'gap-1 px-2.5 py-1.5'}
		{disabled || !phone
