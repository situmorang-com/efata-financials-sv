...
export const POST: RequestHandler = async ({ params, request }) => {
  try {
    ...
    if (batch.type === "special") {
      return json(
        { error: "Special batch does not use saturdays" },
        { status: 400 },
      );
    }
    if (saturdays < 0 || saturdays > batch.total_saturdays) {
      return json(
        { error: `saturdays must be between 0 and ${batch.total_saturdays}` },
        { status: 400 },
      );
    }
    const count = batchItemDb.bulkUpdateSaturdays(
      Number(params.id),
      saturdays,
      batch,
    );
    return json({
      message: `Updated ${count} items to ${saturdays} saturdays`,
      count,
    });
  } catch (error) {
