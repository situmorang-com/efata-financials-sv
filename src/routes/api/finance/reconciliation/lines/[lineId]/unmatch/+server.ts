...
export const POST: RequestHandler = async ({ params, request }) => {
  try {
    ...
    const success = financeReconciliationDb.unmatchLine(lineId, body?.note);
    if (!success) return json({ error: "Line not found" }, { status: 404 });
    return json({ success: true });
  } catch (error) {
    console.error("Error unmatching reconciliation line:", error);
    return json({ error: "Failed to unmatch reconciliation line" }, { status: 500 });
  }
};
