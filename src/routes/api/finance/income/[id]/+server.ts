...
export const PUT: RequestHandler = async ({ params, request, locals }) => {
  try {
    ...
    if (data.party_id !== undefined) payload.party_id = data.party_id ? Number(data.party_id) : null;
    if (data.category_id !== undefined) payload.category_id = Number(data.category_id);
    if (data.account_id !== undefined) payload.account_id = data.account_id ? Number(data.account_id) : null;
    ...
    const result = financeTransactionDb.updateIncomeWithPolicy(id, payload, {
      reason: data.edit_reason,
      actor_email: session?.user?.email ?? null
    });
    if (!result.success) {
      return json({ error: result.error || 'Failed to update income transaction' }, { status: result.status || 400 });
    }

    return json(result.item || financeTransactionDb.getById(id));
  } catch (error) {
